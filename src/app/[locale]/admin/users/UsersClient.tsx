...
export default function UsersPageClient({ locale, users }: { locale: string; users: Profile[] }) {
    ...
    <div className="p-5 md:p-8 max-w-2xl mx-auto space-y-5">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-ink-2 mt-0.5">{users.length} {isAr ? 'حساب' : 'accounts'}</p>
        </div>
      ...
      {creating && (
        <form onSubmit={handleCreate} className="bg-teal-pale border border-teal/20 rounded-2xl p-5 space-y-3">
          ...
          <div className="grid grid-cols-2 gap-3">
            <input name="name_en" required placeholder="Full name (EN)"
          ...
          <select name="role" className="w-full text-sm bg-white rounded-xl border border-border px-3 py-2.5 focus:outline-none focus:ring-2 focus:ring-teal/20">
